...
const addPaginationArrows = (blogSettings) => {
  ...
  previous.setAttribute('data-is-arrow', '');
  next.setAttribute('data-is-arrow', '');
...
const setPaginationArrowPages = (newPageNum, blogSettings) => {
  ...
  // reset the pagination links active state
  blogSettings.paginationContainer.childNodes.forEach((node) => {
    if (node.dataset.page == newPageNum) {
...
const pageChange = (event, blogSettings) => {
  ...
  blogSettings.emitter.emit('JSONBlogPageUpdate', newPageNum, numToDisplay, articles);
}
