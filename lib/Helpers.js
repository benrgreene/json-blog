const addBlogElements = (handle, container) => {
  ...
  newPaginationContainer.setAttribute('id', `jsonblog-pagination-${handle}`);
...
export default {
  defaultSettings: (settings) => {
    ...
    if (!settings.handle) { throw new Error('setting parameter "handle" (string) is not defined!'); }
    ...
    const blogElements = addBlogElements(settings.handle, settings.container);
